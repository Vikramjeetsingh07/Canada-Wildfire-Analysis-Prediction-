...
estimator = GBTClassifier(featuresCol='features', labelCol='in_modis', maxIter=25, maxDepth=10, stepSize=0.1, subsamplingRate=0.8)

wildfire_pipeline = Pipeline(stages=[vecAssembler, estimator])
wildfire_model = wildfire_pipeline.fit(train)

wildfire_model.write().overwrite().save('wildfire_model_gradient')
